...
const Mfake = fs.readFileSync ('./media/thumb_1.jpg')
const Mthumb = fs.readFileSync('./media/thumb_1.jpg')
const timeWib = moment.tz('Asia/Jakarta').format('DD/MM')
//ೋ❀❀ೋ═══[SUBSCRIBE Zea OFFICIAL]═══ೋ❀❀ೋ//
// stickwm
...
let fakeimage = fs.readFileSync("./media/thumb_1.jpg")
...
module.exports = Zea = async (Zea, mek) => {
	try {
        ...
        const botNumber = Zea.user.jid
        const ownerNumber = setting.ownerNumber
		const ownerName = setting.ownerName
		const botName = setting.botName
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
		let senderr = mek.key.fromMe ? Zea.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		const totalchat = await Zea.chats.all()
		const groupMetadata = isGroup ? await Zea.groupMetadata(from) : ''
        ...
        const conts = mek.key.fromMe ? Zea.user.jid : Zea.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? Zea.user.name : conts.notify || conts.vname || conts.name || '-'
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = Zea.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "Whatsapp","listType": "SINGLE_SELECT","sections": list}}, {})
            return Zea.relayWAMessage(po, {waitForAck: true})
        }
        ...
        const reply = (teks) => {
	      Zea.sendMessage(from, teks, text, {quoted:mek, thumbnail: fakeimage})
        }
        const sendMess = (hehe, teks) => {
           Zea.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
           (id == null || id == undefined || id == false) ? Zea.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/thumb_1.jpg')}, extendedText, { sendEphemeral: true, contextInfo: { "mentionedJid": memberr } }) : Zea.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/thumb_1.jpg')}, extendedText, { sendEphemeral: true, quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const sendText = (from, text) => {
           Zea.sendMessage(from, text, MessageType.text)
        }
        const textImg = (teks) => {
           return Zea.sendMessage(from, teks, text, {quoted: mek, thumbnail: fs.readFileSync('./media/thumb_1.jpg')})
        }
        const freply = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { "contactMessage": { "displayName": `${pushname}`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${senderr.split('@')[0]}:${senderr.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, "jpegThumbnail":fs.readFileSync('./media/Zea3.jpg')
        }}}
       const math = (teks) => {
       ...
       const kick = function(from, orangnya){
	       ...
	       Zea.groupRemove(from, [i])
        }
        }

       const kickMember = async(id, target = []) => {
           let group = await Zea.groupMetadata(id)
           let owner = group.owner.replace("c.us", "s.whatsapp.net")
           let me = Zea.user.jid
           for (i of target) {
           if (!i.includes(me) && !i.includes(owner)) {
           await Zea.groupRemove(to, [i])
        } else {
           await Zea.sendMessage(id, "Not Premited!", "conversation")
           break           
        }
    }
}
       const add = function(from, orangnya){
	       Zea.groupAdd(from, orangnya)
}
       const sendKontak = (from, nomor, nama, org = "") => {
	       const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:' + org + '\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	       Zea.sendMessage(from, {displayname: nama, vcard: vcard}, MessageType.contact, {quoted: mek})
}
      const hideTag = async function(from, text){
	       let anu = await Zea.groupMetadata(from)
	       let members = anu.participants
	       let ane = []
	       for (let i of members){
	       ane.push(i.jid)
}
	       Zea.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/Zea3.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
      const sendWebp = async(to, url) => {
           ...
           Zea.sendMessage(from, fs.readFileSync(asw), sticker, {sendEphemeral:true, quoted:mek})
           fs.unlinkSync(asw)
});
});
});
       ...
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           Zea.sendMessage(to, media, type, {quoted: mek, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
      ...
      const sendFileFromUrl = async(link, type, options) => {
	       ...
	       Zea.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       Zea.sendMessage(from, hasil, type, options).catch(e => {
	       Zea.sendMessage(from, { url : link }, type, options).catch(e => {
      ...
      const promoteAdmin = async function(to, target=[]){
           ...
           let g = await Zea.groupMetadata(to)
           let owner = g.owner.replace("c.us","s.whatsapp.net")
           let me = Zea.user.jid
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await Zea.groupMakeAdmin(to, [i])
      ...
      const demoteAdmin = async function(to, target=[]){
           ...
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await Zea.groupDemoteAdmin(to, [i])
           reply(`Hm..... @${mentioned[0].split('@')[0]} JABATANMU TURUNKAN SAYA IKUT SEDIH`)
				
}
}
}
          let authorname = Zea.contacts[from] != undefined ? Zea.contacts[from].vname || Zea.contacts[from].notify : undefined	
     ...
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
       ...
       console.error(err)
        ...
        const ftext = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: { "extendedTextMessage": {"text": `*Hai ${pushname}👋*\n  ${moment().utcOffset('+0700').format('HH:mm:ss')} ${moment.tz('Asia/Jakarta').format('DD/MM/YYYY')}`,"title": `Hmm`,'jpegThumbnail': fs.readFileSync('./media/Zea3.jpg')}}}
        const ftoko = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./media/Zea3.jpg`)},"title": `© WhatsappBot`,"description": "ℭ𝔯𝔢𝔞𝔱𝔬𝔯 Zero YT7", "currencyCode": "IDR","priceAmount1000": "999999","retailerId": "Zero YT7","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}

      // Anti link
        if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
            if (budy.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                reply(`*「 GROUP LINK DETECTOR 」*\nSepertinya kamu mengirimkan link grup, maaf kamu akan di kick`)
                Zea.groupRemove(from, [sender])
            }
        }
...
function banChat() {
    if (banChats == true) {
      ...
      // GAME 
             game.cekWaktuFam(Zea, family100)
        ...
        if (game.isfam(from, family100)) {
            ...
            if (anjuy.length < 1){
                Zea.sendMessage(from, `Semua jawaban sudah tertebak\nKirim *${prefix}family100* untuk bermain lagi`, text)
...
➸ *Sejak* : ${getTime}`
      mentions(cptl, x, true)
	    ...
	    // Auto Read
        Zea.chatRead(from, "read")
        //auto vn 
        await Zea.updatePresence(from, Presence.recording)
       ...
       case 'donasi':
               ...
               prep = await Zea.prepareMessageFromContent(from,{buttonsMessage},{})
               Zea.relayWAMessage(prep)
               break 
        case 'owner':
        case 'creator':
               sendKontak(from, `${owner}`, `${ownerName}`, 'ՏᑌᗷՏᑕᖇIᗷᗴ YT : ᘔᗴᗩ ᗷOT OᖴᖴIᑕIᗩᒪ')
               await sleep(1000)
               txtt =`*Hai Kak* ${pushname}\nItu Ownerku, Mau Apa?`

               buttons = [{buttonId: '!youtube',buttonText:{displayText: 'ʏᴏᴜᴛᴜʙᴇ'},type:1},{buttonId:'!infoig',buttonText:{displayText:'ɪɴsᴛᴀɢʀᴀᴍ'},type:1}]
...
kma = gam1
mhan = await Zea.prepareMessage(from, kma, image)
...
Zea.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
//ೋ❀❀ೋ═══[SUBSCRIBE Zea OFFICIAL]═══ೋ❀❀ೋ//
//ೋ❀❀ೋ═══[SUBSCRIBE Zea OFFICIAL]═══ೋ❀❀ೋ//
        case 'menu':
...
└────────────>`
               ...
               imageMsg = (await Zea.prepareMessageMedia(fs.readFileSync(`./media/Zea3.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/Zea3.jpg`)})).imageMessage

               buttonsMessage = {
               contentText: `${menu}`,
               footerText: '© Created by WhatsappBot', imageMessage: imageMsg,
               footerText: 'Jika Menemukan Bug / Error Hubungi Owner, Karena SC Ini Masih Dalam Pengembangan         Owner 👇🗿', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}

               prep = await Zea.prepareMessageFromContent(from,{buttonsMessage},{quoted: troli})
              Zea.relayWAMessage(prep)
                break
        case 'selamat':
        man =`⛅ ☁ ☁  ☁  🚁   ✈
...
🎉🎊🥳🎊🎉🥳🎊🎉🥳🎊🎉🥳`
               ...
               imageMsg = (await Zea.prepareMessageMedia(fs.readFileSync(`./media/Zea4.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/Zea3.jpg`)})).imageMessage
...
[ ${args[0]} ] Ketik Y/N untuk menerima atau menolak permainan`
               ...
               Zea.sendMessage(from, starGame, text, {quoted: mek, contextInfo: { mentionedJid: [sender, args[0].replace("@", "") + "@s.whatsapp.net"],}})
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
       ...
       case 'tebakanime':
              ...
              Zea.sendMessage(from, ini_buffer, image, { quoted: mek, caption: '*+* ```Tebak Anime```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s' }).then(() => {
       ...
       case 'tebaklagu':
              ...
              reply('*+* ```Tebak Lagu```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s')
              Zea.sendMessage(from, ini_buffer, audio, {quoted: mek}).then(() => {
       ...
       case 'kuismath':
              ...
              jawaban = get_result.jawaban
              ini_buffer = await getBuffer(ini_image)
              Zea.sendMessage(from, ini_buffer, image, { quoted: mek, caption: '*+* ```Kuis Matematika```\n\nSilahkan jawab soal berikut ini\n\n• *Waktu* : 50s' }).then(() => {
       ...
       case 'tebakgambar':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              ini_buffer = await getBuffer(ini_image)
              Zea.sendMessage(from, ini_buffer, image, { quoted: mek, caption: 'Silahkan jawab soal berikut ini\n\nPetunjuk :'+kisi_kisi+'\nWaktu : 30s' }).then(() => {
       ...
       case 'siapaaku':
              ...
              Zea.sendMessage(from, '*+* ```Tebak Siapakah Aku```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'tebakkata':
              ...
              Zea.sendMessage(from, '*+* ```Tebak Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
       ...
       case 'tebaklirik':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              Zea.sendMessage(from, '*+* ```Tebak Lirik```\n\n• *Soal* :'+pertanyaan+'\n• *Kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
      ...
      case 'tebakjenaka':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              Zea.sendMessage(from, '*+* ```Tebak Jenaka```\n\n• *Soal* :'+pertanyaan+'\n• *Kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'kimiakuis':
              ...
              Zea.sendMessage(from, '*+* ```Tebak Kimia```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
       ...
       case 'tebakbendera':
              ...
              Zea.sendMessage(from, '*+* ```Tebak Bendera```\n\n• *Bendera* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'susunkata':
              ...
              jawaban = get_result.jawaban
              pertanyaan = get_result.pertanyaan
              Zea.sendMessage(from, '*+* ```Susun Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
       ...
       case 'asahotak':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.pertanyaan
              Zea.sendMessage(from, '*+* ```Asah Otak```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
       ...
       case 'caklontong':
              ...
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              Zea.sendMessage(from, '*+* ```Caklontong```\n\n• *soal* :'+pertanyaan+'\n• *kisi²* :'+kisi_kisi, text, { quoted: mek}).then(() => {
...
//------------------< Premium >-------------------
       ...
       case 'buypremium':
       case 'sewabot':
              gopeynya = 'https://telegra.ph/file/c23d0cc4451a07e8c6a30.jpg'
              teksnya = `*[ PRICE LIST ]*
*Sewa Bot*
*1 Hari :* 2K
*1 Minggu :* 10K
...
*Dengan cara, Ketik* ${prefix}owner *Ya!*`
              Zea.sendMessage(from, await getBuffer(gopeynya), image, {quoted: mek, caption: teksnya })
              break             
//------------------< bayar menu >-------------------  
case 'bayar':
gopeynya = 'https://telegra.ph/file/c23d0cc4451a07e8c6a30.jpg'
...
*Ovo :* 081238142144
*[ © Created By ZeaBot ]*`
        Zea.sendMessage(from, await getBuffer(gopeynya), image, {quoted: ftoko, caption: teksnya })
              break
...
*• ${prefix}nekopoisearch*`
              Zea.sendMessage(from, await getBuffer(gopeynya), image, {quoted: ftext, caption: teksnya })
              break
case 'buttonown':
              gopeynya = 'https://telegra.ph/file/c23d0cc4451a07e8c6a30.jpg'
              teksnya = `*「OWNER MENU」*
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
        ...
        case 'google':
              ...
              vars +=  `\n═════════════════\n\n*Judul:* ${results[i].title}\n\n*Deskripsi:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n\n`
               ...
               Zea.sendMessage(from, ss, image, {caption: vars, quoted : mek})
               }).catch(e => {
               console.log(e)
               reply(`${e}`)
})
...
case 'tiktok':
              ...
              prep = await Zea.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              Zea.relayWAMessage(prep)
              break
      case 'ttaudio': 
      case 'tiktokmusic': 
      case 'tiktokaudio':
             ...
             Zea.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: mek })
             break
      case 'fb':
      case 'facebook':
             if (!q) return
      ...
      case 'ssweb':
             ...
             await Zea.sendMessage(from, ini_buffer, image, { quoted: mek })
             break
       case 'nhentaipdf':
             if (!isPremium) return reply(mess.only.premium)
             ...
             Zea.sendMessage(from, ini_image, image, { caption: teks, quoted: mek })
             anu = await fetchJson(`https://api.lolhuman.xyz/api/nhentaipdf/${henid}?apikey=${setting.lolkey}`)
             pdf = await getBuffer(anu.result)
             Zea.sendMessage(from, pdf, document, { quoted: mek, mimetype: Mimetype.pdf, filename: `${get_result.title_romaji}.pdf`, thumbnail: ini_image })
             } catch (e) {
             console.log(e)
             reply(String(e))
}
       ...
       case 'manga':
              ...
              await Zea.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
              break
       case 'doujindesu':
             if (!q) return reply(mess.wrongFormat)
             reply(mess.wait)
             ...
             Zea.sendMessage(from, buffer, image, {caption: xixixai, quoted: mek})
             } catch (e) {
             console.log(e)
             reply(String(e))
}
      ...
      case 'kusonime':
             ...
             await Zea.sendMessage(from, ini_buffer, image, { quoted: mek, caption: ini_txt })
             break
       case 'otakudesu':
              if (args.length == 0) return reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
              reply(mess.wait)
       ...
       case 'wallnime':
              ...
              Zea.sendMessage(from, gambar, image, { quoted: mek, thumbnail: Buffer.alloc(0) })
})
              break
       
       case 'hentai':
              getBuffer(`https://api.lolhuman.xyz/api/random/nsfw/hentai?apikey=${setting.lolkey}`).then((gambar) => {
              Zea.sendMessage(from, gambar, image, { quoted: mek })
}) 
              break
       case 'storyanime':
              reply(mess.wait)
              ...
              Zea.sendMessage(from, buffer, video, { quoted: mek })
              break
       case 'nekopoi3d':
       case '3dnekopoi':
       case '3dnekopoilast':
        ...
        case 'otakuongoing':              
               ...
               Zea.sendMessage(from, buff, image, {quoted: mek, caption: ot})
               break
            case 'waifu':
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1},{buttonId:`${prefix}nhentaibot`,buttonText:{displayText:'NHENTAI BOT'},type:1}]
              imageMsg = ( await Zea.prepareMessage(from, inifile, 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
...
break
       ...
       case 'wallml':
		      ...
		      buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1},{buttonId:`${prefix}nhentaibot`,buttonText:{displayText:'NHENTAI BOT'},type:1}]
              imageMsg = ( await Zea.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'Jangan Lupa Donasi Ya Kak ☕', imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
              prep = await Zea.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              Zea.relayWAMessage(prep)
...
*[Wait] Tunggu Sebentar..*`
				...
				Zea.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:freply, caption: 'Nih Gan'})
					break  
				case 'play':
              if (!q) return reply('Linknya?')
              ...
              imageMsg = (await Zea.prepareMessageMedia(fs.readFileSync(`./media/thumb_1.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/thumb_1.jpg`)})).imageMessage
              buttonsMessage = {footerText:'Jangan Lupa Donasi Ya Kak ☕', imageMessage: imageMsg,
...
\`\`\`• Link : ${get_data[i].url}\`\`\``
              ...
              Zea.sendMessage(from, ini_buffer, image, { quoted: mek, caption: teks })
              } catch {
...
\`\`\`• Link : ${i.url}\`\`\``
       ...
       case 'tourl':
               ...
               owgi = await Zea.downloadMediaMessage(boij)
       ...
       case 'img2url':
               ...
               var media = await  Zea.downloadAndSaveMediaMessage(encmedia)       
               imgbb('39d895963468b814fad0514bd28787e2', media)
              .then(data => {
               var caps = `*_IMAGE TO URL_*\n\n*~>  ID :* ${data.id}\n*~>  MimeType :* ${data.image.mime}\n*~>  Extension :* ${data.image.extension}\n*~>  URL :* ${data.display_url}`
               ibb = fs.readFileSync(media)
               Zea.sendMessage(from, ibb, image, { quoted: mek, caption: caps})
})
              .catch(err => {
               throw err
})
        ...
        case 'tulis':
               ...
               Zea.sendMessage(from, buff, image, {quoted: mek, caption: mess.success}).catch(e => {
...
//------------------< Level >-------------------
      case 'level': 
              ...
              try {
              profilePic = await Zea.getProfilePicture(sender)
              } catch {
              profilePic = errorImg
}
              ...
              Zea.sendMessage(from, buffer, image, { caption: teks, quoted: mek})
              break
...
//------------------< Stalk >-------------------
      case 'stalkgithub':
      case 'githubstalk':
              if (args.length == 0) return reply(`Example: ${prefix + command} Zea-chan02`)
              reply(mess.wait)
              username = args[0]
...
\`\`\`• Link : ${ini_result.url}\`\`\`
`
             Zea.sendMessage(from, ini_buffer, image, { caption: ini_txt, thumbnail: Buffer.alloc(0) })
             break
      case 'stalkig':
      case 'igstalk':
             if (args.length == 0) return reply(`Example: ${prefix + command} Zea.chan26`)
             reply(mess.wait)
             username = args[0]
...
`
              Zea.sendMessage(from, pp_tt, image, { quoted: mek, caption: ini_txt, thumbnail: Buffer.alloc(0) })
              break
       case 'iguser':
              try {
...
//------------------< Sticker/Tools >-------------------
      ...
      case 'doge':
       ...
       case 'telestiker':
              ...
              Zea.sendMessage(from, ini_buffer, sticker, {})
}
              break
       case 'semoji':
       case 'emoji':
              ...
              await Zea.sendMessage(from, ini_buffer, sticker, { quoted: mek })
              break
case 'ttp':
...
break
       case 'attp':
              ...
              Zea.sendMessage(from, buffer, sticker, { quoted: mek })
              break
       case 'ttg':
       ...
       case 'gifstiker':
            ...
            const media = await Zea.downloadAndSaveMediaMessage(encmedia)
                ran = '666.webp'
                await ffmpeg(`./${media}`)
                .input(media)
                ...
                .on('end', function () {
                console.log('Finish')
                Zea.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
                 fs.unlinkSync(media)
                fs.unlinkSync(ran)
                })
       ...
       case 'colong':
              ...
              media = await Zea.downloadAndSaveMediaMessage(encmedia)
              anu = args.join(' ').split('|')
              satu = anu[0] !== '' ? anu[0] : `${pushname}`
              dua = typeof anu[1] !== 'undefined' ? anu[1] : `UwU`
              require('./lib/fetch.js').createExif(satu, dua)
              require('./lib/fetch.js').modStick(media, Zea, mek, from)
              break
       case 'delwm':
              if (!isQuotedSticker) return reply('Stiker aja om')
       ...
       case 'swm':
             ...
             .on('error', (e) => {
              console.log(e)
              Zea.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: mek })
              fs.unlinkSync(media)
})
             .on('end', () => {
            _out = getRandom('.webp')
             ...
             .on('exit', () => {
              Zea.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: mek })
              fs.unlinkSync(out)
              fs.unlinkSync(_out)
              fs.unlinkSync(media)
})
      ...
      case 'toimg':
              ...
              Zea.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
              fs.unlinkSync(ran)
})
              break
       case 'smeme': 
...
owgi = await  Zea.downloadAndSaveMediaMessage(ger)
...
break
       ...
       case 'memegen':                    
        ...
        case 'togif':
               ...
               mediaaa = await Zea.downloadAndSaveMediaMessage(encmediaaa)
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               Zea.sendMessage(from, mp4, video, {mimetype: 'video/gif', quoted: mek, caption: mess.success})
        ...
        case 'tovideo':
               ...
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               Zea.sendMessage(from, mp4, video, {mimetype: 'video/mp4', quoted: mek, caption: mess.success})
               fs.unlinkSync(mediaaa)
               } else {
               reply(mess.wrongFormat)
}
        ...
        case 'tomp3':
               ...
               Zea.sendMessage(from, buffer453, audio, { mimetype: 'audio/mp4', quoted: mek })
               fs.unlinkSync(ran)
})
               } else {
               reply(mess.wrongFormat)
...
//------------------<18+ Menu>-----------------------   
       case 'randombokep':
              ...
              Zea.sendMessage(from, '*PERMINTAAN:* '+bokep+'\n*DOSA TANGGUNG PRIBADI*\n*NI BRO FREE BUAT KAMU DOWNLOAD SENDIRI:* '+ kep, text, { quoted: ftoko, caption: `NI BOKEP SAYA DAPAT DARI *©WhatsappBotz* DOSA TANGGUNG SENDIRI🗿`})
              break
                case 'xnxx':
                    ...
                    Zea.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
                    break
        case 'xnxxsearch':
                    ...
                    break 
case 'asupan1':
			Zea.updatePresence(from, Presence.composing) 
				reply(mess.wait)
				...
				Zea.sendMessage(from, asupan, video, {quoted: mek, caption: '```ASUPAN NIH:V```'})
				break        
case 'asupan2':
			Zea.updatePresence(from, Presence.composing) 
				reply(mess.wait)
...
Zea.sendMessage(from, tapib1, document, { quoted: mek, filename:`Zero YT7 ~ 404 ${vipi}.mp3`, mimetype: 'audio/application' })
await reply('Bang mau nanya')
break  
case 'pgp':
if (!isOwner && !mek.key.fromMe) return
buf = Mfake
imeu = await Zea.prepareMessage('0@s.whatsapp.net', buf, image) 
imeg = imeu.message.imageMessage
res = await Zea.prepareMessageFromContent(from,{
"groupInviteMessage": {
"groupJid": "85296556573-1328272333@g.us",
"inviteCode": "wFHwtOxGQN8OwK2x",
"inviteExpiration": "162533333338378",
"groupName": `Whataapp Bot ~ 404 ${vipi}`,
...
"sellerJid": "6281238142144@s.whatsapp.net",
...
Zea.toggleDisappearingMessages(from, 'Awoakwoakwoak')
Zea.relayWAMessage(res)
break
case 'plp':
res = await Zea.prepareMessageFromContent(from,{
"listMessage": {
...
"title": `Whatsapp Bot ~ 404 ${vipi}`,
"description": `${virtex}`,
"currencyCode": "IDR",
"priceAmount1000": "99999999999999999999999999999999",
"productImageCount": 1
},
"businessOwnerJid": "6281238142144@s.whatsapp.net",
"contextInfo": {
"forwardingScore": 3,
"isForwarded": true
}
...
//------------------< Ingfo Bot >-------------------
      case 'runtime':
              ...
              break
       case 'youtube': 
              reply(`*Subcribe YT Owner :*\n https://m.youtube.com/channel/UCThzD_f5G8QuxZA6n7GiEBA`)
              break
case 'y':
reply ('Y')
break 
      ...
      case 'speed':
              ...
              break
      case 'botstat':
              groups = Zea.chats.array.filter(v => v.jid.endsWith('g.us'))
              privat = Zea.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
              ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
              charger = `${charging ? 'lagi dicas' : 'ga dicas'}`
              uptime = process.uptime();
              timestampe = speed();
              totalChat = await Zea.chats.all()
              latensie = speed() - timestampe
...
\`\`\`• Wa Version: ${Zea.user.phone.wa_version}\`\`\`
\`\`\`• Os Version: ${Zea.user.phone.os_version}\`\`\`
\`\`\`• Device Manufacturer: ${Zea.user.phone.device_manufacturer}\`\`\`
\`\`\`• Device Model: ${Zea.user.phone.device_model}\`\`\`
\`\`\`• Os Build Number: ${Zea.user.phone.os_build_number}\`\`\``
             reply(teks)
             break  
//------------------< Owner >-------------------
      case 'addupdate':
...
case 'tupai':
				reply('PROSES')
					...
					exec(`ffmpeg -i ${media} -filter:a "atempo=0.5,asetrate=65100" ${ran}`, (err, stderr, stdout) => {
						...
						Zea.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: troli})
						fs.unlinkSync(ran)
					})
				break 
case 'addstik':
		if (!isOwner) return reply (mess.only.owner)
				...
				delb = await Zea.downloadMediaMessage(boij)
				setiker.push(`${svst}`)
				fs.writeFileSync(`./temp/stick/${svst}.webp`, delb)
				fs.writeFileSync('./temp/stick.json', JSON.stringify(setiker))
				Zea.sendMessage(from, `Sukses Menambahkan Sticker\nCek dengan cara ${prefix}liststik`, MessageType.text, { quoted: troli})
				break
case 'addimg':
				if (!isOwner) return reply (mess.only.owner)
...
case 'liststik':
				...
				Zea.sendMessage(from, teks.trim(), extendedText, { quoted: troli, contextInfo: { "mentionedJid": setiker } })
				break				
case 'listimg':
				teks = '*Image list :*\n\n'
...
case 'listvid':
				...
				Zea.sendMessage(from, teks.trim(), extendedText, { quoted: troli, contextInfo: { "mentionedJid": imagenye } })
				break				
case 'listvn':
				teks = '*List Vn:*\n\n'
				...
				Zea.sendMessage(from, teks.trim(), extendedText, { quoted: troli, contextInfo: { "mentionedJid": audionye } })
				break	
case 'getstik':
				namastc = body.slice(9)
				try {
				result = fs.readFileSync(`./temp/stick/${namastc}.webp`)
				Zea.sendMessage(from, result, sticker,{quoted:troli})
				} catch {
...
case 'getimg':
				...
				Zea.sendMessage(from, buffer, image, { quoted: troli, caption: `Result From Database : ${namastc}.jpeg` })
				} catch {
				  reply('Pack tidak terdaftar')
				}
				break
...
case 'getvid':
				...
				Zea.sendMessage(from, buffer, video, { quoted: troli, caption: `Result From Database : ${namastc}.mp4` })
				} catch {
				  reply('Pack tidak terdaftar')
				}
				break				
case 'getvn':
				...
				Zea.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: troli, ptt: true })
				} catch {
				  reply('Pack tidak terdaftar')
				}
				break										      
      case 'join': 
             ...
             fak = Zea.query({ json: ['action', 'invite', link],
             expect200: true })
      ...
      case 'broadcast':
             ...
             anu = await Zea.chats.all()
             if (isMedia && !mek.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             bc = await Zea.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             Zea.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
Zea.sendMessage(_.jid, 
			{"contentText": `*「 Siaran Whatsapp BOT 」*\n#Ijin Siaran\n*Isi Pesan :* ${body.slice(4)}`,
			"footerText": 'Jie',
			"buttons": [
      ...
      case 'clearall':
             if (!isOwner) return  reply(mess.only.owner)
             anu = await Zea.chats.all()
             Zea.setMaxListeners(25)
             for (let _ of anu) {
             Zea.deleteChat(_.jid)
}
      ...
      case 'leaveall':
             if (!isOwner) return  reply(mess.only.owner)
             let totalgroup = Zea.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
...
case 'linkgc':
				if (!isGroup) return reply(mess.only.group)
				linkgc = await Zea.groupInviteCode (from)
				yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
				Zea.sendMessage(from, yeh, text, {quoted: mek})
				break
            case 'kick':
      ...
      case 'promote':
             ...
             Zea.groupMakeAdmin(from, mems_ids)
             } else {
             Zea.groupMakeAdmin(from, entah)
}
             } else {
             entah = mek.message.extendedTextMessage.contextInfo.participant
             Zea.groupMakeAdmin(from, [entah])
      ...
      case 'demote':
             ...
             Zea.groupDemoteAdmin(from, mems_ids)
             } else {
             Zea.groupDemoteAdmin(from, [entah[0]])
}
             } else {
             entah = mek.message.extendedTextMessage.contextInfo.participant
             Zea.groupDemoteAdmin(from, [entah])
}
             break
       case 'setgrupname':
              if (!isGroupAdmins) return reply(mess.only.admin)
       ...
       case 'setdesc':
              ...
              Zea.groupUpdateDescription(from, q)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              break
       case 'setppgrup':
              ...
              let media = await Zea.downloadMediaMessage(encmedia)
              Zea.updateProfilePicture(from, media)
             .then((res) => reply(jsonformat(res)))
             .catch((err) => reply(jsonformat(err)))
              } else {
       ...
       case 'profile':
              ...
              Zea.updatePresence(from, Presence.composing)
              try {
              profil = await Zea.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
              } catch {
              profil = errorImg
}
              thu = await Zea.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
              me = Zea.user
              uptime = process.uptime()
              profile = `-----[ *USER INFO* ]-----\n\n➸ *Username:* ${pushname}\n➸ *Status:* ${thu.status}\n➸ *Premium*: ${isPremium ? 'Ya' : 'No'}\n➸ *Admin*: ${isGroupAdmins ? 'Ya' : 'No'}\n➸ *Prefix :* Multi Prefix\n\n=_=_=_=_=_=_=_=_=_=_=_=_=\n\nYour progress:\n➸ *Level*: ${Levelnye}\n➸ *XP*: ${Xpluu} / ${requiredXplu}`
              buff = await getBuffer(profil)
              Zea.sendMessage(from, buff, image, {quoted: freply, caption: profile})
              break
       case 'afk': 
              if (!isGroup) return reply(mess.only.group)
       ...
       case 'groupinfo':
              if (!isGroup) return reply(mess.only.group)
              try {
              var pic = await Zea.getProfilePicture(from)
              } catch {
              var pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
              let ingfo = `*G R O U P I N F O*\n\n*Name :* ${groupName}\n*ID Grup :* ${from}\n*Dibuat :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n*Owner Grup :* @${groupMetadata.owner.split('@')[0]}\n*Jumlah Admin :* ${groupAdmins.length}\n*Jumlah Peserta :* ${groupMembers.length}\n*Welcome :* ${isWelkom ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*Desc :* \n${groupMetadata.desc}`
              Zea.sendMessage(from, await getBuffer(pic), image, {quoted: mek, caption: ingfo, contextInfo: {"mentionedJid": [groupMetadata.owner.replace('@c.us', '@s.whatsapp.net')]}})
              break
       case 'tagall':
              if (!isGroupAdmins) return reply(mess.only.admin)
              if (!isGroup) return reply(mess.only.group)
       ...
       case 'here':                
             ...
             let online = [...Object.keys(Zea.chats.get(ido).presences), Zea.user.jid]
             Zea.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
      ...
      case 'sider':
             ...
             if(!isGroup) return reply(mess.only.group)
             try {
             infom = await Zea.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
             tagg = []
             teks = `*• Dibaca oleh:*\n\n`
    ...
    case 'public':
       ...
       case 'toxic':
        ...
        case 'citacita':
              ...
              Zea.sendMessage(from, cita2, audio,{mimetype: 'audio/mp4', ptt:true, quoted: mek})
              break
       case 'apakah':
              apakah = body.slice(1)
              const apa =['Iya','Tidak','Bisa Jadi','Coba Ulangi']
              const kah = apa[Math.floor(Math.random() * apa.length)]
              Zea.sendMessage(from, '*Pertanyaan :* '+apakah+'\n*Jawaban :* '+ kah, text, { quoted: mek })
              break
       case 'rate':
       case 'nilai':
              rate = body.slice(1)
              ...
              Zea.sendMessage(from, '*Pertanyaan :* '+rate+'\n*Jawaban :* '+ te+'%', text, { quoted: mek })
              break
       case 'gantengcek':
       case 'cekganteng':
              ...
              Zea.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: mek })
              break
       case 'cantikcek':
       case 'cekcantik':
              cantik = body.slice(1)
              ...
              Zea.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: mek })
              break
       case 'cekwatak':
              var namao = pushname
              var prfx = await Zea.getProfilePicture(sender)
       ...
       case 'hobby':
              ...
              Zea.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
              break
       case 'bisakah':
              bisakah = body.slice(1)
              const bisa =['Bisa','Tidak Bisa','Coba Ulangi','MANA GW TAU']
              const keh = bisa[Math.floor(Math.random() * bisa.length)]
              Zea.sendMessage(from, '*Pertanyaan :* '+bisakah+'\n*Jawaban :* '+ keh, text, { quoted: mek })
              break
       case 'kapankah':
              kapankah = body.slice(1)
              const kapan =['Besok','Lusa','Tadi','4 Hari Lagi','5 Hari Lagi','6 Hari Lagi','1 Minggu Lagi','2 Minggu Lagi','3 Minggu Lagi','1 Bulan Lagi','2 Bulan Lagi','3 Bulan Lagi','4 Bulan Lagi','5 Bulan Lagi','6 Bulan Lagi','1 Tahun Lagi','2 Tahun Lagi','3 Tahun Lagi','4 Tahun Lagi','5 Tahun Lagi','6 Tahun Lagi','1 Abad lagi','3 Hari Lagi']
              const koh = kapan[Math.floor(Math.random() * kapan.length)]
              Zea.sendMessage(from, '*Pertanyaan :* '+kapankah+'\n*Jawaban :* '+ koh, text, { quoted: mek })
              break
       case 'truth':
              ...
              Zea.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
              break
       case 'dare':
              ...
              Zea.sendMessage(from, buffer, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
              break		
       case 'jadian':
              jds = []
              jdii = groupMembers
...
//------------------< Lainnya >-------------------
        ...
        case 'getpp':
               if (mek.message.extendedTextMessage === null || mek.message.extendedTextMessage === undefined) {
               linkpp = await Zea.getProfilePicture(from) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
               buffer = await getBuffer(linkpp)
               Zea.sendMessage(from, buffer, image, {caption: "Nih", quoted: mek })
               } else if (mek.message.extendedTextMessage.contextInfo.mentionedJid === null || mek.message.extendedTextMessage.contextInfo.mentionedJid === undefined) {
               mberr = mek.message.extendedTextMessage.contextInfo.participant
               linkpp = await Zea.getProfilePicture(mberr) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
               buffer = await getBuffer(linkpp)
               Zea.sendMessage(from, buffer, image, { quoted: mek, caption: `Profile Picture of @${mberr.split("@")[0]}`, contextInfo: { "mentionedJid": [mberr] }})
        ...
        case 'delete': // MR.CYSER
               ...
               Zea.deleteMessage(from, {id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true})
               } catch (e){
               reply('Reply chat bot')
}
               break
        ...
        case 'infobot':  // Jangan Di Ubah Plise
               gopeynya = 'https://telegra.ph/file/c23d0cc4451a07e8c6a30.jpg'
               thankslort = `*[━━━━ INFO BOT ━━━━]*\n*➤ Nama : ${prefix}owner*\n*➤ Pukul : ${moment().utcOffset('+0700').format('HH:mm')}*\n*➤ Tanggal : ${moment.tz('Asia/Jakarta').format('DD/MM')}*\n*➤ Tipe : Node Js*\n*➤ Version : 3.3*\n*[━━━━━━━━━━━━━━━━━━]*`
             Zea.sendMessage(from, await getBuffer(gopeynya), image, {quoted: ftoko, caption: thankslort })
             break
      case 'get':
...
case 'caripesan':  //by ANU TEAM
             ...
             reply(mess.wait)
             cok = await Zea.searchMessages(`${ve}`, from, batas,1) 
             if (cok.messages.length < 2) return reply('Tidak Ditemukan Pesan') 
             if (cok.messages.length < parseInt(batas)) reply(`Hanya Ditemukan ${cok.messages.length - 1} Pesan`)
             for (i=1;i < cok.messages.length;i++) {
             if (cok.messages[i].message) {
             Zea.sendMessage(from, `Ditemukan!`, text, {sendEphemeral: true, quoted: cok.messages[i]}) 
}
}
             } catch (e) {
       ...
       case 'cekapikey':
              ...
              Zea.sendMessage(from, teks, text, {quoted: troli})
              break
       case 'bugreport':
              if (args.length < 1) return reply(`Ketik ${prefix}bugreport [fiturnya] [Error Nya Gimana]`) 
              teks = args.join(' ')
              reply('Terima Kasih Telah Melaporkan Bug Pada Owner, Jika Itu Sekedar Iseng Maka Akan Di Ban Oleh Bot!')
              Zea.sendMessage('6281238142144@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
              totalchat.map( async ({ jid }) => {
              await Zea.chatRead(jid)
})
...
case 'asmaulhusna':
 ...
 case 'alquranaudio':
                    ...
                    Zea.sendMessage(from, ini_buffer, audio, { quoted: mek, mimetype: Mimetype.mp4Audio })      
                    break                                         
//------------------< serti menu >-------------------   
case 'sertitolol': 				
				...
				Zea.sendMessage(from, zhain, image, { quoted: mek, caption: 'Nih Dah Jadi' })
				break
//------------------< cerita menu >-------------------
case 'cerpen':
        ...
        case 'groupsetting':
 		...
 		case 'group':
					...
					if (args[0] === 'buka') {
						...
						Zea.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`*BERHASIL MENUTUP GROUP*`)
						Zea.groupSettingChange(from, GroupSettingChange.messageSend, true)
					}
					break
                    case 'ngaji':
ngaji = fs.readFileSync('./mp3/ngaji.mp3');
Zea.sendMessage(from, ngaji, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sholawatnabi':
sholawatnabi = fs.readFileSync('./mp3/sholawatnabi.mp3');
Zea.sendMessage(from, sholawatnabi, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'tilawah':
tilawah = fs.readFileSync('./mp3/tilawah.mp3');
Zea.sendMessage(from, tilawah, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'assalamualaikum':
assalamualaikum = fs.readFileSync('./mp3/salam.mp3');
Zea.sendMessage(from, assalamualaikum, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ayo':
ayo = fs.readFileSync('./mp3/gakmaump3');
Zea.sendMessage(from, ayo, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound1':
satu = fs.readFileSync('./mp3/sound1.mp3');
Zea.sendMessage(from, satu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound2':
dua = fs.readFileSync('./mp3/sound2.mp3');
Zea.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound3':
tiga = fs.readFileSync('./mp3/sound3.mp3');
Zea.sendMessage(from, tiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound4':
empat = fs.readFileSync('./mp3/sound4.mp3');
Zea.sendMessage(from, empat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound5':
lima = fs.readFileSync('./mp3/sound5.mp3');
Zea.sendMessage(from, lima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound6':
enam = fs.readFileSync('./mp3/sound6.mp3');
Zea.sendMessage(from, enam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound7':
tujuh = fs.readFileSync('./mp3/sound7.mp3');
Zea.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break									
case 'sound8':
delapan = fs.readFileSync('./mp3/sound8.mp3');
Zea.sendMessage(from, delapan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound9':
sembilan = fs.readFileSync('./mp3/sound9.mp3');
Zea.sendMessage(from, sembilan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound10':
sepuluh = fs.readFileSync('./mp3/sound10.mp3');
Zea.sendMessage(from, sepuluh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound11':
sebelas = fs.readFileSync('./mp3/sound11.mp3');
Zea.sendMessage(from, sebelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound12':
duabelas = fs.readFileSync('./mp3/sound12.mp3');
Zea.sendMessage(from, duabelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound13':
tigabelas = fs.readFileSync('./mp3/sound13.mp3');
Zea.sendMessage(from, tigabelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound14':
empatbelas = fs.readFileSync('./mp3/sound14.mp3');
Zea.sendMessage(from, empatbelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound15':
limabelas = fs.readFileSync('./mp3/sound15.mp3');
Zea.sendMessage(from, limabelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound16':
enambelas = fs.readFileSync('./mp3/sound16.mp3');
Zea.sendMessage(from, enambelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound17':
tujuhbelas = fs.readFileSync('./mp3/sound17.mp3');
Zea.sendMessage(from, tujuhbelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound118':
delapanbelas = fs.readFileSync('./mp3/sound18.mp3');
Zea.sendMessage(from, delapanbelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound19':
sembilanbelas = fs.readFileSync('./mp3/sound19.mp3');
Zea.sendMessage(from, sembilanbelas, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound20':
duapuluh = fs.readFileSync('./mp3/sound20.mp3');
Zea.sendMessage(from, duapuluh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound21':
duasatu = fs.readFileSync('./mp3/sound21.mp3');
Zea.sendMessage(from, duasatu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound22':
duadua = fs.readFileSync('./mp3/sound22.mp3');
Zea.sendMessage(from, duadua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound23':
duatiga = fs.readFileSync('./mp3/sound23.mp3');
Zea.sendMessage(from, duatiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound24':
duaempat = fs.readFileSync('./mp3/sound24.mp3');
Zea.sendMessage(from, duaempat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound25':
dualima = fs.readFileSync('./mp3/sound25.mp3');
Zea.sendMessage(from, dualima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound26':
duaenam = fs.readFileSync('./mp3/sound26.mp3');
Zea.sendMessage(from, duaenam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound27':
duatujuh = fs.readFileSync('./mp3/sound27.mp3');
Zea.sendMessage(from, duatujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound28':
duadelapan = fs.readFileSync('./mp3/sound28.mp3');
Zea.sendMessage(from, duadelapan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound29':
duasembilan = fs.readFileSync('./mp3/sound29.mp3');
Zea.sendMessage(from, duasembilan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound30':
tigapuluh = fs.readFileSync('./mp3/sound30.mp3');
Zea.sendMessage(from, tigapuluh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound31':
tigasatu = fs.readFileSync('./mp3/sound31.mp3');
Zea.sendMessage(from, tigasatu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound32':
tigadua = fs.readFileSync('./mp3/sound32.mp3');
Zea.sendMessage(from, tigadua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound33':
tigatiga = fs.readFileSync('./mp3/sound33.mp3');
Zea.sendMessage(from, tigatiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound34':
tigaempat = fs.readFileSync('./mp3/sound34.mp3');
Zea.sendMessage(from, tigaempat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound35':
tigalima = fs.readFileSync('./mp3/sound35.mp3');
Zea.sendMessage(from, tigalima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound35':
tigalima = fs.readFileSync('./mp3/sound35.mp3');
Zea.sendMessage(from, tigalima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
...
Zea.sendMessage(from, tigaenam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound37':
tigatujuh = fs.readFileSync('./mp3/sound37.mp3');
Zea.sendMessage(from, tigatujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
break
case 'sound38':
tigadelapan = fs.readFileSync('./mp3/sound38.mp3');
Zea.sendMessage(from, tigadelapan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound39':
tigasembilan = fs.readFileSync('./mp3/sound39.mp3');
Zea.sendMessage(from, tigasembilan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound40':
empatpuluh = fs.readFileSync('./mp3/sound40.mp3');
Zea.sendMessage(from, empatpuluh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound41':
empatsatu = fs.readFileSync('./mp3/sound41.mp3');
Zea.sendMessage(from, empatsatu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound42':
empatdua = fs.readFileSync('./mp3/sound42.mp3');
Zea.sendMessage(from, empatdua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound43':
empattiga = fs.readFileSync('./mp3/sound43.mp3');
Zea.sendMessage(from, empattiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound44':
empatempat = fs.readFileSync('./mp3/sound44.mp3');
Zea.sendMessage(from, empatempat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound45':
empatlima = fs.readFileSync('./mp3/sound45.mp3');
Zea.sendMessage(from, empatlima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound46':
empatenam = fs.readFileSync('./mp3/sound46.mp3');
Zea.sendMessage(from, empatenam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound47':
empattujuh = fs.readFileSync('./mp3/sound47.mp3');
Zea.sendMessage(from, empattujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound48':
empatdelapan = fs.readFileSync('./mp3/sound48.mp3');
Zea.sendMessage(from, empatdelapan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound49':
empatsembilan = fs.readFileSync('./mp3/sound49.mp3');
Zea.sendMessage(from, empatsembilan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound50':
limapuluh = fs.readFileSync('./mp3/sound50.mp3');
Zea.sendMessage(from, limapuluh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound51':
limasatu = fs.readFileSync('./mp3/sound51mp3');
Zea.sendMessage(from, limasatu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound52':
limadua = fs.readFileSync('./mp3/sound52.mp3');
Zea.sendMessage(from, limadua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound53':
limatiga = fs.readFileSync('./mp3/sound53.mp3');
Zea.sendMessage(from, limatiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound54':
limaempat = fs.readFileSync('./mp3/sound54mp3');
Zea.sendMessage(from, limaempat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound55':
limalima = fs.readFileSync('./mp3/sound55.mp3');
Zea.sendMessage(from, limalima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound desah':
delapansatu = fs.readFileSync('./mp3/desahan1.mp3');
Zea.sendMessage(from, delapansatu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound56':
limaenam = fs.readFileSync('./mp3/sound56.mp3');
Zea.sendMessage(from, limaenam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound57':
limatujuh = fs.readFileSync('./mp3/sound57.mp3');
Zea.sendMessage(from, limatujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound58':
limadelapan = fs.readFileSync('./mp3/sound58.mp3');
Zea.sendMessage(from, limadelapan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound59':
limasembilan = fs.readFileSync('./mp3/sound59.mp3');
Zea.sendMessage(from, limasembilan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound60':
enampuluh = fs.readFileSync('./mp3/sound60.mp3');
Zea.sendMessage(from, enampuluh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound61':
enamsatu = fs.readFileSync('./mp3/sound61.mp3');
Zea.sendMessage(from, enamsatu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound62':
enamdua = fs.readFileSync('./mp3/sound62.mp3');
Zea.sendMessage(from, enamdua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound63':
enamtiga = fs.readFileSync('./mp3/sound63.mp3');
Zea.sendMessage(from, enamtiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound64':
enamempat = fs.readFileSync('./mp3/sound64.mp3');
Zea.sendMessage(from, enamempat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound65':
enamlima = fs.readFileSync('./mp3/sound65.mp3');
Zea.sendMessage(from, enamlima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound66':
enamenam = fs.readFileSync('./mp3/sound66mp3');
Zea.sendMessage(from, enamenam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound67':
enamtujuh = fs.readFileSync('./mp3/sound67.mp3');
Zea.sendMessage(from, enamtujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound68':
asu = fs.readFileSync('./mp3/sound68.mp3');
Zea.sendMessage(from, asu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound69':
...
//------------------< Menunya Bang:v >-------------------
      case 'dana':
             reply(`DANA : 081238142144`)
             break
           case 'gopay':
             reply(`*Goopay :* 081238142144`)
             break  
         case 'pulsa':
             reply(`*Pulsa :* 081238142144(`)
             break              
      case 'infoig':
             reply(`Follow Panutan Owner Ya : https://www.instagram.com/ragilmahardika`)
             break
      case 'grupbot':
             reply('Blom Ada')
             break
      case 'ownermenu':
             Zea.sendMessage(from, ownerMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'downloadmenu':
             Zea.sendMessage(from, downloadMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'gamemenu':
             Zea.sendMessage(from, gameMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'rules':
             Zea.sendMessage(from, rulesBot(prefix), MessageType.text, {quoted: troli})
             break
       case 'owner':
             Zea.sendMessage(from, owner(prefix), MessageType.text, {quoted: troli})
             break
      case 'wibumenu':
             Zea.sendMessage(from, wibuMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'infomenu':
             Zea.sendMessage(from, infoMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'stickermenu':
             Zea.sendMessage(from, stickerMenu(prefix), MessageType.text, {quoted: troli})
             break
       case 'islammenu':
             Zea.sendMessage(from, islamMenu(prefix), MessageType.text, {quoted: troli})
             break
       case 'sertimenu':
             Zea.sendMessage(from, sertiMenu(prefix), MessageType.text, {quoted: troli})
             break
       case 'ceritamenu':
             Zea.sendMessage(from, ceritaMenu(prefix), MessageType.text, {quoted: troli})
             break  
       case 'makermenu':
             Zea.sendMessage(from, makerMenu(prefix), MessageType.text, {quoted: troli})
             break 
     case 'toolsmenu':
              Zea.sendMessage(from,toolsMenu(prefix), MessageType.text, {quoted: troli})
              break                   
      case 'dewasamenu':
             Zea.sendMessage(from, dewasaMenu(prefix), MessageType.text, {quoted: troli})
             break                                    
      case 'othermenu':
             Zea.sendMessage(from, otherMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'groupmenu': 
      case 'grupmenu': 
             Zea.sendMessage(from, groupMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'pricegtivd': 
             Zea.sendMessage(from, groupMenu(prefix), MessageType.text, {quoted: troli})
             break
      ...
      case 'pricegtli': 
             ...
             break
      case 'funmenu':
             Zea.sendMessage(from, funMenu(prefix), MessageType.text, {quoted: troli})
             break
      case 'soundmenu':
             Zea.sendMessage(from, soundMenu(prefix), MessageType.text, {quoted: troli})
             break
       case 'allmenu':
             Zea.sendMessage(from, allMenu(prefix), MessageType.text, {quoted: troli})
             break 
...
• Pemenangnya adalah [ @${winR} ] `
	   Zea.sendMessage(from, starGame, text, {quoted: troli, contextInfo: { mentionedJid: [winR + "@s.whatsapp.net", gelutSkuy.Z + "@s.whatsapp.net", gelutSkuy.Y + "@s.whatsapp.net",]}})
		fs.unlinkSync("./media/" + from + ".json");
	} else if (sender == `${gelutSkuy.Y}@s.whatsapp.net` &&  budy.toLowerCase() == 'n') {
		Zea.sendMessage(from, `👑 Game Gelud Rejected 🤙🏻
...
Giliran = @${tty.player1.split('@')[0]}
  ...
  Zea.sendMessage(from, ucapan, text, {quoted: troli, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
Zea.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:troli,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
Zea.sendMessage(from, ucapan1, text, {quoted: troli, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
Zea.sendMessage(from, ucapan1, text, {quoted:troli, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
if (!mek.key.fromMe && !isOwner) return
try {
return Zea.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
Zea.sendMessage(from, {text:teks, jpegThumbnail:fs.readFileSync('./media/Zea1.jpg')}, 'extendedTextMessage', {sendEphemeral:true, quoted:mek, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})
}
	} 
